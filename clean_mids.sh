...
do
	...
	adapter_trim="_AT"
	filename_out1=$filename$adapter_trim
	...
	echo "Percent of nucleotides trimmed as tags/adapters $delta2 %" >> mid1_6_cleaning_data.txt
	echo "	" >> mid1_6_cleaning_data.txt
	...
	filename_out2=$filename$adapter_trim$trim_edge$fastq
	...
	trim_quality -q $quality_thresh -o $filename_out3 $filename$adapter_trim$trim_edge$vector_trim$fastq

	num_nuc_start4=$(sed -n 2~4p $filename$adapter_trim$trim_edge$vector_trim$fastq | tr -d '\n' | wc -m)
	...
	echo "Total contigs: $num_contigs" >> mid1_6_cleaning_data.txt
	echo "Largest contig: $largest_contig" >> mid1_6_cleaning_data.txt
	echo "	" >> mid1_6_cleaning_data.txt
	echo "#-------------------------------------------------------------------------------#" >> mid1_6_cleaning_data.txt

	assembly_data="_Asm_Data.tsv"
	echo "Contig_num	Contig_len	Contig_cov" > $filename$adapter_trim$trim_edge$vector_trim$quality_clip$assembly_data
	echo "$contig_stats" >> $filename$adapter_trim$trim_edge$vector_trim$quality_clip$assembly_data
